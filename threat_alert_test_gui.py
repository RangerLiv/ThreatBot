...
from PIL import Image, ImageTk  # For alert icon swapping
import pygame

pygame.mixer.init()

# Example list of fake threat alerts with geo data
fake_alerts = [
...
class ThreatAlertGUI:
    def __init__(self, root):
        ...
        self.live_feed_running = True  # Track if feed is active
        ...
        self.simulate_alert()  # Show an alert instantly on startup
    ...
    # Filter out alerts that don’t match the selected severity
        if self.severity_filter != "All" and alert["severity"] != self.severity_filter:
            return

    # Format timestamp and insert into alert area
    ...
    # Play a sound depending on alert type
        if self.sound_on:
    ...
    # Update map with threat location
    ...
    # Flash the left frame red for visual emphasis
        self.left_frame.config(bg="red")
        self.root.after(100, lambda: self.left_frame.config(bg=self.bg_color))

    # Pop up CVE or threat-related info
        self.show_cve_popup(alert["message"])

    # Change icon based on severity
        if alert["severity"] == "Critical":
            new_icon = self.keyblade_icon  # Make sure this is loaded in __init__
        else:
            new_icon = self.shield_img

            self.icon_label.configure(image=new_icon)
            self.icon_label.image = new_icon  # Keep a reference to avoid garbage collection

    # Revert icon to shield after 10 seconds
    ...
    def get_random_interval(self):
        return random.randint(180000, 1800000)  # Between 3 min and 30 min
...
# Line ~185+
    ...
    def toggle_live_feed(self):
        ...
        if self.live_feed_running:
            ...
            self.simulate_live_feed()  # Resume the feed loop
        else:
